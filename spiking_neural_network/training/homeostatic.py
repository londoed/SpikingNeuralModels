...
class HomeostaticSTDP(STDP):
    ...
    def calculate_firing_rate(self, neuron):
        if neuron.fired:
            neuron.firing_count += 1
    ...
    def update_potentiation(self, neuron):
        ...
        neuron.weights = np.add(neuron.weights,
                                np.multiply(np.add(self.average_potentiation,
    ...
    def update_depression(self, neuron):
        v = np.random.normal(self.gaussian_mean, self.gaussian_sd)
        neuron.weights = np.add(neuron.weights,
                            np.multiply(np.add(np.multiply(
                                -self.average_depression, neuron.weights),
                                    np.multiply(v, neuron.weights)), np.exp(1 / self.tau_stdp)))

    def activity_dependent_scaling(self, neuron):
        neuron.weights = np.multiply(np.multiply(self.beta, neuron.weights),
                                                (self.activity_goal - neuron.firing_rate))

    def update_weights(self, neuron):
        if neuron.refactory_time is 0:
    ...
    def update(self, layers):
        for layer in layers:
            for neuron in layer:
                ...
                self.calculate_firing_rate(neuron)
